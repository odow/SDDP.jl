...
mutable struct Vertex
    ...
    variable_ref::Union{Nothing,VariableRef}
end

mutable struct InnerConvexApproximation
    theta::VariableRef
    states::Dict{Symbol,VariableRef}
    objective_states::Union{Nothing,NTuple{N,VariableRef} where {N}}
    belief_states::Union{Nothing,Dict{T,VariableRef} where {T}}
...
function _vertex_selection(V::InnerConvexApproximation, solver)
  ...
  for j = 1:nvertices
    v = V.vertices[j]
    for k in state_keys
      set_normalized_rhs(cc_x[k], v.state[k])
    end
    set_normalized_rhs(cc_t, v.value)
...
function SDDP.initialize_bellman_function(
    ...
    if length(node.children) != 0
        ...
        @variable(sp, 0 <= σ0 <=1)
        if objective_sense(sp) == MOI.MIN_SENSE
...
function _get_vertex_info(json_vertex, dualcuts;
    ...
    if dualcuts
        ...
        state = Dict(Symbol(vertex_name_parser(k)) => v for (k, v) in json_vertex["coefficients"])
    else
        value = json_vertex["value"]
...
function read_vertices_from_file(
    ...
    for node_info in vertices
        ...
        for json_vertex in list
            value, state, obj_y, belief_y = _get_vertex_info(json_vertex, dualcuts; vertex_name_parser)
            _add_vertex(
                bf.global_theta,
                value,
                state,
        ...
        for json_cut in node_info["risk_set_cuts"]
            ...
            if objective_sense(node.subproblem) == MOI.MIN_SENSE
