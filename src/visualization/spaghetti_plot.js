...
var runmain = function(data) {
    ...
    data.map(function(plotdata, fileindex) {
        ...
        var domain = [
            d3.min(plotdata.data, function(sim) {
                return d3.min(sim)
            }),
            d3.max(plotdata.data, function(sim) {
                return d3.max(sim)
            })
        ];
        if (plotdata.ymin != "") {
            ...
            .on("mousedown", function(yvalues, lineindex) {
                if (isdrag == false) {
                    ...
                    var color = d3.scale.pow().exponent(0.25)
                        .domain([0,
                            Math.max(
                                Math.abs(yvalues[stageindex] - d3.min(plotdata.data, function(sim) {
                                    return sim[stageindex];
                                })),
                                Math.abs(yvalues[stageindex] - d3.max(plotdata.data, function(sim) {
                                    return sim[stageindex];
                                }))
                            )
                        ])
...
var shifthistogram = function(plots, tocolumn) {
    ...
    plots.map(function(plot) {
        ...
        var y = d3.scale.linear()
            .domain([0, d3.max(plot.histdata[tocolumn], function(d) {
                return d.y
            })])
...
function addhistogram(plots, p) {
    ...
    var y = d3.scale.linear()
        .domain([0, d3.max(p.histdata[p.histdata.length - 1], function(d) {
            return d.y
        })])
        .range([maxbarheight, 0]);
    ...
    mybars.append("rect", ".axis")
        ...
        .attr("class", function(d, i) {
            return "bar chart" + p.file + i
        })
        // transform to correct location
        .attr("transform", function(d) {
            return "rotate(180) translate(" + -(width + maxbarheight) + "," + -height + ")";
        })
        // y location of bar
        .attr("y", function(d) {
            return x(d.x);
