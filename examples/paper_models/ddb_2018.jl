...
function example_two()
    ...
    spot_price =
        pretty_plot(simulations; title = "(a) Spot Price", ylabel = "Spot Price\n(\$/MWh)") do data
            return data[:objective_state][1]
        end

    stored_energy =
        pretty_plot(simulations; title = "(b) Total Stored Energy", ylabel = "Volume\n(m³)") do data
            return sum(data[:volume][i].out for i = 1:5)
        end

    total_generation =
        pretty_plot(simulations; title = "(b) Total Generation", ylabel = "Energy\n(MWh)") do data
