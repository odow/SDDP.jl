...
function _solve_example(risk_measure)
    ...
    two_stage_model = SDDP.LinearPolicyGraph(
        ...
        if t == 1
            ...
            P = [p1 * p2 * p3 for (_, p1) in X for (_, p2) in Y for (_, p3) in Z]
...
function _run_instance(risk_measure)
    ...
    model = SDDP.LinearPolicyGraph(
        ...
        @constraint(
            sp, c, stocks.out + bonds.out + consumption - stocks.in - bonds.in == 0
        )
        @stageobjective(sp, consumption)
        if t > 1
            SDDP.parameterize(sp, Ω, P) do ω
                set_normalized_coefficient(c, stocks.in, -ω.s)
                set_normalized_coefficient(c, bonds.in, -ω.b)
            end
        end
    end
    Random.seed!(1234)
    scenarios = [
        [(t, s[t]) for t = 1:5]
        for s in collect(Iterators.product([nothing], Ω, Ω, Ω, Ω))[:]
    ]
    ...
    simulations = SDDP.simulate(
        model, length(scenarios), [:stocks, :consumption];
        sampling_scheme = CyclicHistorical(scenarios, 1)
    )
    ...
    D = Distributions.DiscreteNonParametric(
        collect(keys(data)), collect(values(data))
    )
    return (
...
function build_figure_6()
    ...
    data_entropic = Dict(
        γ => d
        for (γ, d) in map(γ -> (γ, _run_instance(SDDP.Entropic(γ))), γ_entropic)
    )
    ...
    data_avar = Dict(
        γ => d
        for (γ, d) in map(γ -> (γ, _run_instance(SDDP.AVaR(1 - γ))), γ_avar)
    )
...
function _compute_stocks_bonds(risk_measure)
    ...
    scenarios = [
        [(t, s[t]) for t = 1:5]
        for s in collect(Iterators.product([(s = 1.0, b = 1.0)], Ω, Ω, Ω, Ω))[:]
    ]
    probabilities = prod.(collect(Iterators.product([1.0], P, P, P, P))[:])
...
function build_figure_7()
    ...
    open(joinpath(@__DIR__, "figure_7_entropic.dat"), "w") do io
        ...
        _write_matrix(
            io, hcat(0:1:200, [e[1] for e in entropic], [e[2] for e in entropic])
        )
    end
