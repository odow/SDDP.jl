...
for r = 0.0:0.1:0.5
    srand(1234)
    push!(fits, [getfittedvalue(N, r) for i = 1:100])
end
fitted_estimates = hcat(fits...)

using Plots, StatPlots
fntsm = Plots.font("times", 10.0 * Plots.pt)
fntlg = Plots.font("times", 12.0 * Plots.pt)
default(
    titlefont = fntlg,
    guidefont = fntlg,
    tickfont = fntsm,
...
p1 = boxplot(fitted_estimates, title = "(a)", ylabel = "Estimator", ylims = (0.35, 0.65))
p2 = plot(
