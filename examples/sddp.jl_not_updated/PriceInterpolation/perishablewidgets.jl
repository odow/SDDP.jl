...
m = SDDPModel(
    ...
    value_function = StaticPriceInterpolation(
        ...
        dynamics = (price, noise) -> begin
            ...
            new_price = exp((1 - reversion_rate) * log(price) +
                            reversion_rate * log(process_mean) + noise)
    ...
    @constraints(
        ...
        #==
                ...
                ==#
            [i = forward_contracts[1:end-1]],
            contracts[i] == contracts0[i+1] + contract_sells[i]
        ...
        #==
                ...
                ==#
            [i = perishability_periods[2:end]],
            storage[i] == storage0[i-1] - deliver_to_contract[i] - sell_on_spot[i]
        #==
        ...
        #==
            ...
            sum(perishability[p] * sell_on_spot[p] -
                ...
                (1 - perishability[p]) * deliver_to_contract[p] for p in perishability_periods) -
            #==
            ...
            sum(forward_curve[i] * contract_sells[i] for i in forward_contracts)
    ...
    @stageobjective(
        sp,
        price -> price * widget_equivalent_sold -
                 transaction_cost * total_contracts_traded,
    )

end

@time status = SDDP.solve(
    m,
...
SDDP.addplot!(
    ...
    1:200,
    1:12,
    (i, t) -> results[i][:stageobjective][t],
    title = "Accumulated Profit",
    ylabel = "Accumulated Profit (\$)",
    cumulative = true,
)
...
SDDP.addplot!(
    ...
    1:200,
    1:12,
    (i, t) -> results[i][:contract_sells][t][1],
    title = "Contract Sells 1",
)
SDDP.addplot!(
    plt,
    1:200,
    1:12,
    (i, t) -> results[i][:contract_sells][t][2],
    title = "Contract Sells 2",
)
SDDP.addplot!(
    plt,
    1:200,
    1:12,
    (i, t) -> results[i][:contract_sells][t][3],
    title = "Contract Sells 3",
)
SDDP.addplot!(
    plt,
    1:200,
    1:12,
    (i, t) -> results[i][:contract_sells][t][4],
    title = "Contract Sells 4",
)
SDDP.addplot!(
    plt,
    1:200,
    1:12,
    (i, t) -> results[i][:contract_sells][t][5],
    title = "Contract Sells 5",
)
SDDP.addplot!(
    plt,
    1:200,
    1:12,
    (i, t) -> results[i][:contract_sells][t][6],
    title = "Contract Sells 6",
)
...
SDDP.addplot!(
    ...
    1:200,
    1:12,
    (i, t) -> results[i][:sell_on_spot][t][1],
    title = "Spot Sells 1",
)
SDDP.addplot!(
    plt,
    1:200,
    1:12,
    (i, t) -> results[i][:sell_on_spot][t][2],
    title = "Spot Sells 2",
)
SDDP.addplot!(
    plt,
    1:200,
    1:12,
    (i, t) -> results[i][:sell_on_spot][t][3],
    title = "Spot Sells 3",
)
SDDP.addplot!(
    plt,
    1:200,
    1:12,
    (i, t) -> results[i][:sell_on_spot][t][4],
    title = "Spot Sells 4",
)
SDDP.addplot!(
    plt,
    1:200,
    1:12,
    (i, t) -> results[i][:sell_on_spot][t][5],
    title = "Spot Sells 5",
)
SDDP.addplot!(
    plt,
    1:200,
    1:12,
    (i, t) -> results[i][:deliver_to_contract][t][1],
    title = "Contract Delivery 1",
)
SDDP.addplot!(
    plt,
    1:200,
    1:12,
    (i, t) -> results[i][:deliver_to_contract][t][2],
    title = "Contract Delivery 2",
)
SDDP.addplot!(
    plt,
    1:200,
    1:12,
    (i, t) -> results[i][:deliver_to_contract][t][3],
    title = "Contract Delivery 3",
)
SDDP.addplot!(
    plt,
    1:200,
    1:12,
    (i, t) -> results[i][:deliver_to_contract][t][4],
    title = "Contract Delivery 4",
)
SDDP.addplot!(
    plt,
    1:200,
    1:12,
    (i, t) -> results[i][:deliver_to_contract][t][5],
    title = "Contract Delivery 5",
)
