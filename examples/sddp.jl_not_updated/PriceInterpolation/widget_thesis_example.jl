...
if length(ARGS) > 0
    if ARGS[1] == "static"
        ...
        srand(123)
        status = SDDP.solve(m, iteration_limit = 10)
        ...
        for N = 2:20
            ...
            srand(123)
            status = SDDP.solve(m, iteration_limit = 50, print_level = 0)
    ...
    elseif ARGS[1] == "dynamic"
        ...
        srand(123)
        status = SDDP.solve(m, iteration_limit = 500)
    ...
    elseif ARGS[1] == "hybrid"
        ...
        m = newsvendor_example(1)
        srand(123)
        status = SDDP.solve(m, time_limit = 20.0)
        ...
        rm("hybrid.cuts")
        status = SDDP.solve(m, time_limit = 20.0 - offset)
        for l in m.log
