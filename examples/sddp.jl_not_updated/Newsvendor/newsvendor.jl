...
@test SDDP.solve(
    ...
    simulation = MonteCarloSimulation(frequency = 10, min = 10, max = 500, step = 10),
    bound_stalling = BoundStalling(iterations = 5, atol = 1e-3),
) == :bound_stalling
...
@test SDDP.solve(
    ...
    iteration_limit = 50,
    print_level = 0,
    simulation = MonteCarloSimulation(frequency = 10, min = 500, max = 500, step = 1),
) == :iteration_limit
...
news5 = newsvendormodel(
    riskmeasure = EAVaR(beta = 0.6, lambda = 0.5),
    oracle = DematosCutOracle(),
)
