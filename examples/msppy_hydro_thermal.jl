...
function msppy_hydro_thermal()
    ...
    @assert N_THERMAL == length.(thermal_obj) == length.(thermal_lb) == length.(thermal_ub)
    ...
    model = SDDP.LinearPolicyGraph(
        ...
        @variable(
            ...
            SDDP.State,
            initial_value = storedEnergy_initial[i],
        )
        @variables(
            sp,
            begin
                ...
                thermal_lb[i][j] <= thermal[i = 1:4, j = 1:N_THERMAL[i]] <=
                thermal_ub[i][j]
        ...
        @stageobjective(
            sp,
            sum(deficit_obj[i] * sum(deficit[i, :]) for i = 1:4) +
            sum(thermal_obj[i][j] * thermal[i, j] for i = 1:4 for j = 1:N_THERMAL[i]),
        )
        @constraints(
            sp,
            begin
                [i = 1:4],
                sum(deficit[i, :]) + hydroGeneration[i] +
                sum(thermal[i, j] for j = 1:N_THERMAL[i]) + sum(exchange[:, i]) -
                sum(exchange[i, :]) == demand[month][i]
                [i = 1:4],
