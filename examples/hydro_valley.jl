...
function hydrovalleymodel(
    ...
    m = SDDP.MarkovianPolicyGraph(
        ...
        @constraints(
            ...
            # Total quantity generated
                generation_quantity == sum(turbine(r).powerknots[level] *
                                           dispatch[r, level] for r = 1:N for level = 1:length(turbine(r).powerknots))

            # ------------------------------------------------------------------
            # Flow out
                turbineflow[r = 1:N],
                outflow[r] == sum(turbine(r).flowknots[level] * dispatch[r, level] for level = 1:length(turbine(r).flowknots))
        ...
        if hasstagewiseinflows && t > 1 # in future stages random inflows
            ...
            SDDP.parameterize(
                subproblem,
                [(valley_chain[1].inflows[i], valley_chain[2].inflows[i]) for i = 1:length(transition)],
            ) do ω
                for i = 1:N
