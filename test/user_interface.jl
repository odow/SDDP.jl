...
@testset "PolicyGraph constructor" begin
    @testset "LinearGraph" begin
        model =
            SDDP.PolicyGraph(SDDP.LinearGraph(2), lower_bound = 0.0, direct_mode = false) do node,
                stage
            end

        @test_throws Exception SDDP.PolicyGraph(SDDP.LinearGraph(2), lower_bound = 0.0) do node,
            stage
        end
        nodes = Set{Int}()
        model =
...
@testset "Errors" begin
    ...
    @testset "missing bounds" begin
        exception = ErrorException("You must specify a bound on the objective value, through " *
...
@testset "Numerical stability report" begin
    model =
        SDDP.LinearPolicyGraph(stages = 2, lower_bound = -1e10, direct_mode = false) do subproblem,
            t
...
@testset "objective_state" begin
    model =
        SDDP.LinearPolicyGraph(stages = 2, lower_bound = 0, direct_mode = false) do subproblem,
            t
